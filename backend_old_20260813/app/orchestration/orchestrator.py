...
Both endpoints now call into this class instead, so the sequencing lives
in exactly one place, and can be tested (and reused, e.g. by a future
...
leaving them nothing to compare against. This is what makes "reference
answer OR retrieved evidence" -- the original scope for those two agents --
actually true of the running system, not just of code that exists
...
class EvaluationOrchestrator:
    ...
    # RAG fallback
    # ------------------------------------------------------------------
    def _resolve_reference(self, question: str, reference_answer: str) -> tuple[str, dict]:
        """
        ...
        return "", {
            "source": "none",
            "reason": "No reference_answer supplied and no retrievable evidence "
    ...
    # Full pipeline
    # ------------------------------------------------------------------
    def run_all(self, question: str, response: str, reference_answer: str = "") -> dict:
        resolved_reference, rag_info = self._resolve_reference(question, reference_answer)

        relevance_result = self.relevance_agent.evaluate(question, response)
        accuracy_result = self.accuracy_agent.evaluate(response, resolved_reference)
        hallucination_result = self.hallucination_agent.evaluate(response, resolved_reference)
        completeness_result = self.completeness_agent.evaluate(question, response)
    ...
    def run_verdict_only(self, question: str, response: str, reference_answer: str = ""):
        ...
        return self.run_all(question, response, reference_answer)["verdict"]
